...
fn main() {
	...
	// 测试3: 百万次路由匹配 - 大规模路由
	test_million_matches_large_scale()
...
fn test_million_matches_large_scale() {
	println('\n📊 百万次路由匹配 - 大规模 (100个动态路由)...')
	...
	// 添加100个动态路由
	for i in 0 .. 100 {
		route := '/api/v${i}/users/:user_id/posts/:post_id'
		handler := hono.ContextHandler{
			path: route
			handler: fn (mut c hono.Context) http.Response {
				return c.text('test')
	...
	test_paths := [
		...
		'/api/v25/users/789/posts/101',
		'/api/v50/users/111/posts/222',
		'/api/v75/users/333/posts/444',
		'/api/v99/users/555/posts/666'
	]
